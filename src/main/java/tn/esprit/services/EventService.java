...
public class EventService implements IService<Event> {
    ...
    public int add(Event event) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = cnx.prepareStatement(req, Statement.RETURN_GENERATED_KEYS)) {
            ...
            preparedStatement.setDate(2, new java.sql.Date(event.getDateDebut().getTime())); // Conversion java.util.Date -> java.sql.Date
            preparedStatement.setDate(3, new java.sql.Date(event.getDateFin().getTime()));   // Conversion java.util.Date -> java.sql.Date
            ...
            if (rs.next()) {
                event.setId(rs.getInt(1)); // Récupérer l'ID généré pour l'événement ajouté
            }
    ...
    public void ajouterAssociationEvenementSponseur(int eventId, int sponseurId) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = cnx.prepareStatement(query)) {
            ...
            // Vous pouvez effectuer un rollback ici si nécessaire
            throw e; // Re-lancer l'exception pour informer le contrôleur de l'erreur
        }
    }


    public void addSponseurToEvent(int eventId, int sponseurId) throws SQLException {
        String req = "INSERT INTO event_sponseur (event_id, sponseur_id) VALUES (?, ?)";
        try (PreparedStatement preparedStatement = cnx.prepareStatement(req)) {
            preparedStatement.setInt(1, eventId);
            preparedStatement.setInt(2, sponseurId);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            System.out.println("Erreur lors de l'ajout du sponsor à l'événement : " + e.getMessage());
            throw e;
        }
    }
